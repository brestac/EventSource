...
void EventSource::Event::print() {
  DEBUG_PRINTF("[Event]  type: '%s', origin: '%s', data: '%s', lastEventId: '%s', "
               "error: '%s', code: %d",
               type, origin, message.data, message.lastEventId,
...
void EventSource::update() {
  ...
  if (_initial_connection) {
    ...
    } else if (_sseAutoreconnect &&
               (millis() - _lastConnectionTime) > _retryDelay * _retryDelayMultiplier) {
...
void EventSource::_onDataStatic(void *arg, AsyncClient *client,
                                 void *data, size_t len) {
  static_cast<EventSource *>(arg)->_onData(client, data, len);
}

void EventSource::_onErrorStatic(void *arg, AsyncClient *client, uint8_t error) {
...
void EventSource::_connect_client() {
  DEBUG_PRINTF("[SSE] Connexion à %s:%hu%s retry:%u\n",
               _apiHost, _apiPort, _ssePath, _retryDelay);
